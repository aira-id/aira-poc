"""
G2P (Grapheme-to-Phoneme) conversion for Indonesian language.

This module converts Indonesian text to phonetic representation using:
- BERT-based pronunciation prediction for ambiguous 'e' sounds
- Dictionary-based lookup for common words
- Syllable-based stress and alophone rules
- Phonetic mapping to IPA symbols
"""
...
class Predictor:
    """
    BERT-based phoneme predictor for Indonesian vowel 'e' disambiguation.

    Uses a trained BERT model to predict the correct pronunciation of 'e' vowels
    in Indonesian words, which can be pronounced as /e/, /ɛ/, or /ə/.
    """
...
class G2P:
    """
    Grapheme-to-Phoneme converter for Indonesian language.

    Converts Indonesian text to phonetic representation using a combination of:
    - Dictionary lookup for common words
    - BERT-based prediction for ambiguous vowels
    - Syllable-based stress assignment
    - Alophone rules for natural pronunciation
    - Phonetic mapping to IPA symbols

    The converter handles:
    - Letter-by-letter spelling (abjad)
    - Word pronunciation with proper stress
    - Alophone variations based on syllable position
    - Final consonant adjustments
    """

    def __init__(self):
        """Initialize G2P converter with dictionary, predictor, and syllable splitter."""
